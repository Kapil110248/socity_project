...
export default function HelpdeskTicketsPage() {
  ...
  const filteredTickets = useMemo(() => {
    ...
    return data.filter((ticket: any) => {
      ...
      // 3. Tab filter (Status)
      if (activeTab !== 'all' && ticket.status?.toLowerCase() !== activeTab.toLowerCase()) {
        return false
      }
  ...
  const stats = useMemo(() => {
    ...
    const getCount = (status: string) => data.filter((t: any) => t.status?.toLowerCase() === status.toLowerCase()).length
